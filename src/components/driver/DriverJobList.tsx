...
function JobCard({ order, kind }: { order: OrderWithRelations; kind: "pickup" | "delivery" }) {
  ...
  return (
    <div className={`rounded-2xl bg-card p-4 shadow-sm ${late ? "ring-2 ring-danger" : ""}`}>
      ...
      {/* Customer phone: tap to call, plus a WhatsApp shortcut. */}
      {order.customer?.phone && (
        <div className="mt-2 flex items-center gap-2">
          ...
          <a
            ...
            className="ms-auto flex items-center gap-1.5 rounded-lg bg-[#25D366] px-3 py-1.5 text-xs font-bold text-white"
          >
