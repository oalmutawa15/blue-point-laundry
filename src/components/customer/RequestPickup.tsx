...
export function RequestPickup({ addresses }: { addresses: Tables<"addresses">[] }) {
  ...
  if (addresses.length === 0) {
    return (
      <div className="rounded-3xl bg-brand p-6 text-center text-brand-foreground shadow-sm">
        <p className="text-lg font-extrabold">{t.home.requestPickup}</p>
        <p className="mt-2 text-sm text-white/80">{t.home.needAddressFirst}</p>
        <Link
          href="/addresses"
          className="mt-4 inline-block rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-brand"
        >
          {t.addresses.add}
        </Link>
      </div>
